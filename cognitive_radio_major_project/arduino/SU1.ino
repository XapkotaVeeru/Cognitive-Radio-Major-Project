...
#define SU_Address 4       // Set to 4 for SU1
...
// 🏔️ Fallback Values for SU1 (Slightly different from SU2 to easily identify them on the dashboard)
const float DEFAULT_TEMP = 24.0;
const float DEFAULT_HUM = 50.0;
const int DEFAULT_MQ135 = 400;
...
void setup() {
  ...
  Serial.print("✅ SU1 Ready (Address ID: ");
...
void sendAccessRequest() {
  ...
  Serial.println("📤 SU1 Requesting Channel...");
      ...
      if (rf95.recv(buf, &len)) {
        ...
        if (msg.startsWith(String(SU_Address) + ",")) {
          ...
          if (newFreq > 0) {
            ...
            Serial.println("✅ SU1 Frequency Allocated: " + String(newFreq) + " MHz");
            return;
          } else if (newFreq == -1) {
...
void sendSensorData() {
  ...
  delay(10);
  String dummyPayload = "SU1_TRANSMITTING_ON_ALLOCATED_SPECTRUM";
  rf95.send((uint8_t*)dummyPayload.c_str(), dummyPayload.length());
  rf95.waitPacketSent(1000);

  Serial.println("📤 SU1 Sent Dashboard Telemetry & Occupied " + String(allocatedFreq) + " MHz");
}
...
void sendEndMessage() {
  ...
  Serial.println("🏁 SU1 Cycle complete, ending LoRa transmission.");
}